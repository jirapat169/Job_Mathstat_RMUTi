...
const FormPersonal = ({
  ...
  const onSubmit = async (data) => {
    ...
    data["index"] = personal.length;
    let cloneData = { ...data };
    delete cloneData["key"];

    if (personalUpdate) {
      // Update DB
      await db(`/personal/${data["key"]}`).set(cloneData);
    } else {
      // Insert DB
      await db("/personal").push(cloneData);
    }
...
export default class personal extends Component {
  ...
  async componentDidMount() {
    ...
    window.$("#personalModal").on("hidden.bs.modal", (e) => {
      this.setState({ personalModal: false });
    });

    window.$("#personalModal").on("show.bs.modal", (e) => {
      this.setState({ personalModal: true });
    });
  }
      ...
      <>
          ...
          <div className="col-md-6 text-right mb-3">
            <span className="mr-3">
              <button
                ...
                onClick={() => {
                  this.setState({
                    personalCopy: this.state.personal.filter(
        ...
        >
          <div className="modal-dialog" role="document">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">เรียงลำดับรายชื่อ</h5>
                <button
                  type="button"
                  className="close"
                  data-dismiss="modal"
              ...
              <div className="modal-body">
                <DndProvider backend={Backend}>
                  {this.state.personalCopy.map((el, i) => {
                    return (
                      <Card
                        key={i}
                        index={i}
                        id={el.key}
                        text={`${el.prefix_th} ${el.name_th}`}
                        moveCard={(dragIndex, hoverIndex) => {
                          const dragCard = this.state.personalCopy[dragIndex];
                          this.setState({
                            personalCopy: update(this.state.personalCopy, {
                              $splice: [
                                [dragIndex, 1],
              ...
              <div className="modal-footer">
                ...
                <button
                  ...
                  onClick={async () => {
                    let backup = {};
                    let data = [...this.state.personalCopy];

                    this.state.personalCopy.forEach((el, i) => {
                      data[i]["index"] = i;
                    ...
                    this.state.personal
                      ...
                      .forEach((el, i) => {
                        data2[i]["index"] = i;
                        backup2[data2[i]["key"]] = { ...data2[i] };
                        delete backup2[data2[i]["key"]]["key"];
                      });

                    let backup3 = {};
                    let data3 = [
                      ...this.state.personal.filter(
                        (v) => v.position.indexOf("ธุรการ") > -1
                      ),
                    ];
                    this.state.personal
                      .filter((v) => v.position.indexOf("ธุรการ") > -1)
                      .forEach((el, i) => {
                        data3[i]["index"] = i;
                        backup3[data3[i]["key"]] = { ...data3[i] };
                        delete backup3[data3[i]["key"]]["key"];
                      });

                    await this.props
                      .db("/personal")
                      .set({ ...backup, ...backup2, ...backup3 });
                    this.loadData();
